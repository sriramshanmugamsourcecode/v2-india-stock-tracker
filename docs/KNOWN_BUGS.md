...
- **Cost basis not reset on full exit → re-buy (2026-08-28, `v4.20260828.10`)**: `calcPortfolio()` computed `avgBuy = totalCost / buyQty` as a lifetime sum over every BUY ever; SELLs only touched `sellQty`/`netQty`, never the buy-side pool. So after buy → sell-all → re-buy, the new position's average was a blend of the old closed lot and the new lot (e.g. Buy 10@100, Sell 10, Buy 5@200 → showed ₹133.33 instead of ₹200); `invested` and `realised P&L` (booked against the blended average, not the basis at sale time) were wrong too. Fixed with a chronological **moving-average cost** pass per stock in `calcPortfolio()`: trades sorted by `date` then `created_at`/`id`; each SELL books `(price − runningAvg) × units − brokerage` to realised P&L and removes `units × runningAvg` from the cost pool; when qty hits 0 the pool resets, so later buys start clean. Also fixes partial-sell-then-rebuy. XIRR unaffected (uses raw dated cashflows). Stocks never sold (e.g. APARINDS) are unchanged. This changed the core aggregation — done with explicit sign-off after a manual DB backup, not a silent enhancement.
