...
**Ticker format — actually resolved 2026-08-26 (previous note here was wrong, never checked against real data)**: this seed script's `INSERT` statements use exchange-prefixed tickers (`'NSE:POLYCAB'`). A prior version of this doc assumed the live table had already moved to un-prefixed based only on a narrative doc, not a real query. A real authenticated export proved the opposite — the live table was still ~554/559 rows prefixed. This mattered because `handleImportFile`'s `universeMap` build does **not** strip prefixes before matching Kite's bare symbols, so nearly every Kite import was failing to auto-match. Fixed via a one-time data migration (`UPDATE universe SET ticker = split_part(ticker, ':', 2) WHERE ticker LIKE '%:%'`, after resolving 3 collisions — see [[project-overview]] for detail). `universe.ticker` is now confirmed un-prefixed for all 559 rows. `filterStockSearch`'s `.split(':')` defensiveness remains in the code but is now a no-op.
