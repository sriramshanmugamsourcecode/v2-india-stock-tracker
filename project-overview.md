...
- **Universe ticker format — corrected 2026-08-26 against a real authenticated export, not inferred**: the previous version of this note concluded (wrongly) that the live table was already un-prefixed, based only on the seed SQL vs. a narrative doc, never an actual query. A real CSV export of the live table showed the opposite: ~554 of ~559 rows were stored **with** exchange prefix (`'NSE:AARTIIND'`, `'BSE:ARTSON'`, etc.) — and critically, the Kite-import matching code (`handleImportFile`'s `universeMap` build, `index.html` ~3704-3708) does **not** strip prefixes before matching against Kite's bare symbols, so nearly every imported trade was failing to auto-match and falling into the manual stock-picker. This was fixed as a **data-only migration** (`UPDATE universe SET ticker = split_part(ticker, ':', 2) WHERE ticker LIKE '%:%'`), after resolving three collisions the strip would have caused (`BLS`, `TORNTPHARM` — true duplicates, kept the legacy row; `TMCV` — legacy row was stale, labeled "Tata Motors DVR" for shares delisted ~2020-2021, while `TMCV.NS` is actually the current post-2025-demerger Tata Motors Limited entity, so the legacy row was deleted instead). As of this fix, `universe.ticker` is confirmed un-prefixed for all rows (`SELECT count(*) FILTER (WHERE ticker LIKE '%:%')` = 0 of 559). The `filterStockSearch`/`.split(':')` defensive handling in the code remains as legacy safety but is now genuinely a no-op.
- **Known residual gap (not fixed, code untouched per project's no-logic-changes rule)**: the manual-entry stock-search dropdown (`index.html` ~4054-4055) determines exchange only by checking whether `ticker` contains `:`, defaulting to `'NSE'` for any bare ticker — it does not read the `exchange` column. Now that all tickers are bare, this dropdown will always guess NSE regardless of the true exchange. Harmless for the ~552 NSE-listed rows, but means manually adding a trade (via search, not Kite import) for a BSE-only universe entry (currently `ARTSON`, `PGINVIT`, `INDIGRID`) will mistag it as NSE and fetch the wrong Yahoo price. Kite import is unaffected (it reads exchange from the file's own column). Flagged, not fixed — would need explicit sign-off as a logic change.
